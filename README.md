...
shipped with a version of Boost that may collide with the version you are using for compilation. 
...
If you whish to link with the static version of Boost on Linux, Boost should be compiled the following way 
...
## 5 - Known issues
- It is known that the MEX files on OSX does not honour its dependencies (boost) because of a troubleshooting with Matlab. On OSX currently the only possible way to run the MEX file is to use the static boost libraries.
